...
const Avatar = ({ name, size = 40 }: { name: string; size?: number }) => (
  ...
  >
    <span style={{ color: "white", fontSize: size * 0.38, fontWeight: 600 }}>
      {getInitials(name)}
    </span>
  </div>
);
    ...
    <div>
      ...
      {methods.length === 0 ? (
        <p style={{ fontSize: 12, color: "#9CA3AF" }}>
          Không có hình thức nào cho {currency}
        </p>
      ) : (
        <div className="flex flex-wrap gap-2">
          {methods.map((m) => (
            <button
              ...
              style={{
                background: selected.includes(m.id) ? "#EFF6FF" : "#F9FAFB",
                border: `2px solid ${selected.includes(m.id) ? PRIMARY : "#E5E7EB"}`,
                cursor: "pointer",
              }}
            >
              ...
              <span
                style={{
                  fontSize: 12,
                  fontWeight: selected.includes(m.id) ? 700 : 400,
                  color: selected.includes(m.id) ? PRIMARY : "#374151",
                }}
              >
                {m.name}
              </span>
...
function MethodBadge({
  ...
  return (
    <span
      ...
      style={{
        background: highlight ? "#ECFDF5" : "#F3F4F6",
...
function CreateDealModal({
  ...
  const toggleRecipient = (id: string) =>
    setForm((f) => ({
      ...f,
      recipientPaymentMethods: f.recipientPaymentMethods.includes(id)
        ? f.recipientPaymentMethods.filter((x) => x !== id)
  ...
  const changeTo = (code: string) => {
    ...
    setForm((f) => ({
      ...f,
      toCurrency: code,
      recipientPaymentMethods: methods.slice(0, 2).map((m) => m.id),
    }));
  };
  ...
  const validate = () => {
    ...
    if (!form.minAmount || isNaN(Number(form.minAmount)))
      e.minAmount = "Nhập số tiền tối thiểu";
    if (!form.maxAmount || isNaN(Number(form.maxAmount)))
        ...
        >
          ...
          >
            ...
            <PaymentMethodPicker
              ...
              selected={form.recipientPaymentMethods}
    ...
    >
      <div className="p-4 pb-3">
        ...
        <div className="flex items-start gap-3 mb-3">
          ...
          <div className="flex-1">
            <div>
              ...
              <div>
                <Stars rating={req.requesterRating} />
              </div>
            </div>
        ...
        <div className="bg-gray-50 rounded-xl p-3 mb-3">
          ...
          {(() => {
            ...
            return (
              ...
              >
                ...
                <span
                  style={{ fontSize: 11, color: "#9CA3AF", margin: "0 2px" }}
                >
                  →
                </span>
            ...
            <div>
              ...
              {senderMethod && (
                <MethodBadge
                  ...
                  methodId={req.senderPaymentMethod}
                  highlight
                />
              )}
            </div>
            ...
            <div className="text-right">
              ...
              {recipientMethod && (
                <MethodBadge
                  ...
                  methodId={req.recipientPaymentMethod}
                  highlight
                />
              )}
            </div>
          ...
          <p style={{ fontSize: 10, color: "#9CA3AF" }}>
            {fmtRate(req.rate, req.fromCurrency, req.toCurrency)}
          </p>
        </div>
        ...
        >
          ...
          >
            ...
            <ChevronRight
              ...
              color="#065F46"
              style={{
                transform: showRecipient ? "rotate(90deg)" : "rotate(0deg)",
          ...
          <div className="space-y-2">
            ...
            >
              ...
              <div className="space-y-1.5">
                ...
                >
                  💸 Platform fee — thu ngay, không hoàn trả
                </p>
                ...
                >
                  🔒 Escrow — tạm giữ trong suốt giao dịch
                </p>
                <div className="flex items-center justify-between pl-3">
                ...
                >
                  🔓 Escrow giải phóng khi giao dịch hoàn tất.
                </p>
              </div>
            </div>
            )}
        ...
        {req.status === "completed" && (
          ...
          >
            ...
            <p style={{ fontSize: 12, color: "#6B7280", lineHeight: 1.7 }}>
              - Phí hệ thống ({(req.systemFeeRate * 100).toFixed(1)}%):{" "}
              {fmt(feeAmt, req.fromCurrency)}
              <br />- Escrow đã được giải phóng
            </p>
          </div>
        )}
        ...
        {req.status === "disputed" && (
          <div className="space-y-2">
            <div
              className="rounded-xl p-3"
              style={{ background: "#FFF7ED", border: "1.5px solid #FCD34D" }}
            >
              <p style={{ fontSize: 12, fontWeight: 700, color: "#9A3412" }}>
                ⚠️ Khiếu nại bởi:{" "}
                {req.disputedBy === "provider" ? "Bạn" : req.requesterName}
              </p>
              {req.disputeNote && (
                <p
                  style={{
              ...
              <div className="space-y-2 mb-5">
                ...
                <div className="flex justify-between py-2 border-b border-gray-100">
                  ...
                  <span
                    style={{ fontSize: 13, fontWeight: 600, color: "#374151" }}
                  >
                    {getCurrency(req.toCurrency)?.flag}{" "}
                    {(
                      {
                        VND: "Việt Nam",
                        USD: "Mỹ",
...
function RequestsTab({
  ...
  const handleUpdate = (id: string, partial: Partial<DealRequest>) =>
    onRequestsChange(
      requests.map((r) => (r.id === id ? { ...r, ...partial } : r)),
    );
  ...
  useEffect(() => {
    ...
    const target = document.getElementById(
      `provider-request-${selectedRequestId}`,
    );
    if (!target) return;
    window.requestAnimationFrame(() => {
      target.scrollIntoView({ behavior: "smooth", block: "start" });
    });
  }, [selectedRequestId, filter, filtered.length]);
  ...
  return (
    <div className="flex-1 overflow-y-auto">
      ...
      >
        ...
        >
          Quản lý & xử lý các yêu cầu chuyển tiền
        </p>
      </div>
      <div className="flex gap-2 px-4 py-3 border-b border-gray-100 overflow-x-auto">
        {(["pending", "active", "completed", "other"] as ProvReqFilter[]).map(
          (f) => (
            <button
              key={f}
              onClick={() => setFilter(f)}
      ...
      <div className="px-4 py-4 space-y-3">
        {filtered.map((req) => {
          ...
          return (
            <div
              key={req.id}
              id={`provider-request-${req.id}`}
              className={
                isSelected
                  ? "rounded-2xl ring-2 ring-blue-300 ring-offset-2 ring-offset-gray-50"
                  : undefined
              }
            >
              <ProviderRequestCard
      ...
      {view === "list" ? (
        ...
        <div className="flex-1 overflow-y-auto px-4 py-4 space-y-4">
              ...
              {(
                ...
                return (
                  <button
                    ...
                    style={{
                      background: currency === c ? "#EFF6FF" : "#F9FAFB",
                      border: `2px solid ${currency === c ? PRIMARY : "#E5E7EB"}`,
                      cursor: "pointer",
                    }}
                  >
                    <span
                      style={{
                        ...
                        color: currency === c ? PRIMARY : "#374151",
                      }}
                    >
                      {ci?.flag} {c}
                    </span>
              ...
              {(PAYMENT_METHODS_BY_CURRENCY[currency] ?? [])
                ...
                .map((m) => (
                  <button
                    ...
                    style={{
                      background:
                        form.methodId === m.id ? "#EFF6FF" : "#F9FAFB",
                      border: `2px solid ${form.methodId === m.id ? PRIMARY : "#E5E7EB"}`,
                      cursor: "pointer",
                    }}
                  >
                    <span style={{ fontSize: 15 }}>{m.icon}</span>
                    <span
                      style={{
                        ...
                        color: form.methodId === m.id ? PRIMARY : "#374151",
                      }}
                    >
                      {m.name}
                    </span>
          ...
          <button
            ...
            style={{
              background: form.methodId && form.label ? PRIMARY : "#E5E7EB",
              border: "none",
...
function ProfileTab({ onRoleChange }: { onRoleChange: () => void }) {
  ...
  const menuItems = [
    {
      icon: <History size={18} color="#059669" />,
    ...
    {
      icon: <ArrowLeftRight size={18} color="#D97706" />,
  ...
  return (
    <div className="flex-1 overflow-y-auto relative">
      ...
      <div
        style={{ background: `linear-gradient(135deg, ${PRIMARY}, #1D4ED8)` }}
        className="px-5 pt-12 pb-12 relative overflow-hidden"
      >
        ...
        <div className="flex flex-col items-center relative z-10">
          ...
          <h2 style={{ color: "white", fontSize: 22, fontWeight: 700 }}>
            Nguyễn Văn B
          </h2>
      ...
      <div className="px-4 -mt-6 relative z-10 pb-6 space-y-4">
        <div className="grid grid-cols-3 gap-3">
          {[
            { label: "Giao dịch", value: "248", color: PRIMARY },
            { label: "Tháng này", value: "18", color: "#059669" },
            { label: "Tỷ lệ HT", value: "99%", color: "#D97706" },
          ].map((s) => (
            <div
              key={s.label}
        ...
        <div className="bg-white rounded-2xl shadow-sm border border-gray-100 overflow-hidden">
            ...
            <div className="flex items-center gap-3">
              ...
              <div className="text-left">
                ...
                <p style={{ fontSize: 12, color: "#6B7280", marginTop: 1 }}>
                  {accounts.length} tài khoản đã lưu
                </p>
              </div>
            </div>
          ...
          {accounts.length > 0 && (
            <div className="px-4 pb-3 flex flex-wrap gap-2">
              ...
              {accounts.length > 4 && (
                <div
                  className="flex items-center px-2.5 py-1.5 rounded-xl"
                  style={{ background: "#F3F4F6" }}
                >
                  <span style={{ fontSize: 11, color: "#6B7280" }}>
                    +{accounts.length - 4} khác
                  </span>
                </div>
              )}
            </div>
          ...
          {menuItems.map((item, i) => (
            <button
              ...
              style={{
                ...
                cursor: "pointer",
                borderBottom:
                  i < menuItems.length - 1 ? "1px solid #F3F4F6" : "none",
              }}
            >
              <div
                className="w-9 h-9 rounded-xl flex items-center justify-center"
